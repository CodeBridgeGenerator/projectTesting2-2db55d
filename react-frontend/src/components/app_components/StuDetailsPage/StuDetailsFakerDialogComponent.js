...
import stuDetailsFakerFactory from "./stuDetailsFakerFactory";
...
const StuDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stuDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stuDetails").create(elem));
...
export default connect(mapState, mapDispatch)(StuDetailsFakerDialogComponent);
